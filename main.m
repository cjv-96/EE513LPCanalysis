...
funcflag = 1; %function flag wp in whisperme (set this to a # from 0-4 to implement different functionalities)
scalestart = 440;
rectype = {'No Modification','Whisper Reconstruction','Custom Reconstruct','Sinusoid Reconstruct'};
%% debug
% f0 = 440; 
% samps = fs * dur;
%tax = (0:samps)/fs;
...
%% read files
[sig, fso] = audioread('seatbelt.wav'); %signal to be analyzed and reconstructed
lpcorder = 10;
...
while last < length(sig)
	...
	dataseg = dataseg .* std(dataseg);
	dataseg = whisperme(dataseg, fs, noisea, lpcorder, funcflag, mp(first:last).*winshape); %run whisperme
...
figure(35);
subplot(2, 1, 2);
plot(sigout);title(['Reconstructed Signal: ' rectype(funcflag + 1)]);
xlabel('Time (sec)'); ylabel('Amplitude');
